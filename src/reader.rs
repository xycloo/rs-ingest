use stellar_xdr::next::{LedgerCloseMeta, TransactionEnvelope, GeneralizedTransactionSet, TransactionPhase, TxSetComponent, TransactionResultMeta, TransactionResultMetaV1, SorobanTransactionMeta, SorobanTransactionMetaV2, TransactionMeta, ContractEvent};
...
impl LedgerCloseMetaReader {
    ...
    pub fn transaction_metas(result: &MetaResult) -> Result<Vec<GenericTxMetas<TransactionResultMeta, TransactionResultMetaV1>>, ReaderError> {
        let meta = MetaResultReader::read_meta(result)?;

        match meta {
            LedgerCloseMeta::V0(v0) => Ok(v0.tx_processing.to_vec().iter().map(|meta| GenericTxMetas::V0(meta.clone())).collect()),
            LedgerCloseMeta::V1(v1) => Ok(v1.tx_processing.to_vec().iter().map(|meta| GenericTxMetas::V0(meta.clone())).collect()),
            LedgerCloseMeta::V2(v2) => Ok(v2.tx_processing.to_vec().iter().map(|meta| GenericTxMetas::V1(meta.clone())).collect()),
        }
    }

    pub fn soroban_metas(result: &MetaResult) -> Result<Vec<GenericTxMetas<SorobanTransactionMeta, SorobanTransactionMetaV2>>, ReaderError> {
        let mut soroban_metas = Vec::new();

                for result_meta in Self::transaction_metas(result)? {
                    match result_meta {
                        GenericTxMetas::V0(v0) => {
                            match v0.tx_apply_processing {
                        ...
                        GenericTxMetas::V1(v1) => {
                            match v1.tx_apply_processing {
                                TransactionMeta::V0(_) => {
                                    ()
                                }
    ...
    pub fn soroban_events(result: &MetaResult) -> Result<Vec<ContractEvent>, ReaderError> {
        ...
        for meta in soroban_metas {
            match meta {
                GenericTxMetas::V0(v0) => {
                    contract_events.append(&mut v0.events.to_vec())
                },
                GenericTxMetas::V1(_) => {
